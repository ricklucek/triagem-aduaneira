...
export default function PrepostoLookupPanel({
  ...
  const filtered = useMemo(() => {
    ...
    return results.filter((item) =>
      [item.nome, item.contatoNome, item.cidade, item.telefone, item.email]
        .filter(Boolean)
  ...
  return (
    <Card className="gap-4 rounded-2xl border-border/80 p-4 shadow-none sm:p-5">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Consulte os prepostos disponíveis para a localidade selecionada ou
      ...
      <Field
        ...
        hint={
          !cidade.trim()
            ? "Informe a cidade/porto/fronteira para habilitar a consulta."
            : undefined
        }
      >
        <TextInput
          ...
          placeholder="Pesquisar por nome, contato ou localidade"
        />
      </Field>

      <details className="rounded-xl border border-border bg-background px-4 py-3">
        <summary className="cursor-pointer text-sm font-medium">
          Abrir opções encontradas
        </summary>
        <Stack gap={10}>
          <div className="mt-4 grid gap-3">
            {filtered.length === 0 ? (
              <p className="text-sm text-muted-foreground">
                Nenhum preposto encontrado para a consulta atual.
              </p>
            ) : (
              filtered.map((item) => (
                <button
