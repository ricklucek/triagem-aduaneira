...
import type {
  PrepostoLookupResponse,
  PublicCompanyLookupResponse,
} from "@/lib/api/types/public-api";
import axios from "axios";

export const publicApi = {
  async lookupCompanyByCnpj(
    cnpj: string,
  ): Promise<PublicCompanyLookupResponse> {
    const { data } = await axios
      .create({})
      .get<PublicCompanyLookupResponse>(API_ROUTES.public.cnpjLookup(cnpj));
