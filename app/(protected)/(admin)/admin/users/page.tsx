...
export default function AdminUsersPage() {
  ...
  const onSubmit = async (event: FormEvent) => {
    ...
    setForm({ nome: "", email: "", password: "", role: "comercial", setor: "" });
    await mutate();
  };

  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-semibold">Usuários (somente administrador)</h1>
      ...
      <Card>
        ...
        <CardContent>
          <form className="grid gap-3 md:grid-cols-2" onSubmit={onSubmit}>
            ...
            <div className="flex items-end"><Button type="submit">Criar usuário</Button></div>
          </form>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent>
          {isLoading ? <p>Carregando usuários...</p> : (
            <Table>
              <TableHeader><TableRow><TableHead>Nome</TableHead><TableHead>E-mail</TableHead><TableHead>Perfil</TableHead><TableHead>Setor</TableHead><TableHead>Status</TableHead></TableRow></TableHeader>
              <TableBody>
                {data?.map((user) => (
                  <TableRow key={user.id}>
                    <TableCell>{user.nome}</TableCell><TableCell>{user.email}</TableCell><TableCell>{user.role}</TableCell><TableCell>{user.setor}</TableCell><TableCell>{user.ativo ? "Ativo" : "Inativo"}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
